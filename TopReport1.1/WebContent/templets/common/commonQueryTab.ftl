...
<@CommonQueryMacro.DynamicTabSet id=id width=_w height=_w hasMenu="true" selected=currentTab>
	<#list operationsKeys as key>
		...
		<#assign url  = button.attributes["url"]?default("")>
	 	...
	 	<#if _show>
	 		<#if url?contains("?")>
				<#assign _url = url + "&_tab_=${tid}">
			<#else>
				<#assign _url = url + "?_tab_=${tid}">
		 	</#if>
			<@CommonQueryMacro.DynamicTab id=tid title=desc url=_url />
		</#if>
	</#list>
