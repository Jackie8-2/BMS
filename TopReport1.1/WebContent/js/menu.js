...
function Menu_buildMenu(node, bar)// init treeView from array
{
	...
	while (index <= nLen) {
		...
		case 2:
			...
			menutemp.add(tmpItem = new MenuItem(node[index + 1],
					node[index + 7], null, null, node[index + 2]));
			tmpItem.target = "main";
			break;
		case 0:
			count++;
			...
			if (count == 1) {
				...
				var menutemp = menuarray[count - 1];
				menutemp.add(new MenuItem(node[index + 1], null, null,
						menuarray[count], node[index + 2]));
			}
			break;
		}
		index++;
...
/*
 * Menu.prototype.cssText = ".menu-body {background-color:
 * #0257CC;color:white;margin:0;padding:0;
 * overflow:hidden;border:0;cursor:default;}"+ ".menu-body .outer-border
 * {border:1px solid;border-color:ThreeDLightShadow ThreeDDarkShadow
 * ThreeDDarkShadow ThreeDLightShadow;}"+ ".menu-body .inner-border {border:1px
 * solid;border-color:#636563 #636563 #636563 #636563;
 * padding:1px;width:100%;height:100%;}"+ ".menu-body td {font-family: \"??\",
 * Arial;font-size: 12px;}"+ ".menu-body .hover
 * {background-color:blue;color:white;}"+ ".menu-body td.empty-icon-cell
 * {padding: 0px 2px 0px 2px;border:0;}"+ ".menu-body td.empty-icon-cell span
 * {width:5px;}"+ ".menu-body td.icon-cell {padding:0px 2px 0px 2px;border:0;}"+
 * ".menu-body td.icon-cell img {width:16px;height:16px;margin:0;}"+ ".menu-body
 * td.label-cell {width:100%;padding:1px 5px 0px 0px;font-size: 12px;}"+
 * ".menu-body td.shortcut-cell {padding:0px 5px 0px 5px;}"+ ".menu-body
 * td.arrow-cell {width:20px;padding:0px 2px 0px
 * 0px;font-family:Webdings;font-size:80%;}"+ ".menu-body .disabled
 * .disabled-container {color:GrayText;}"+ ".menu-body .disabled .icon-cell
 * .disabled-container,.menu-body .disabled-hover .icon-cell .disabled-container
 * {background:GrayText;filter:progid:DXImageTransform.Microsoft.Chroma(Color=#010101)
 * DropShadow(color=ButtonHighlight, offx=1, offy=1);width:100%;height:100%;}"+
 ...
 * .disabled-container,.menu-body .disabled-hover .icon-cell .disabled-container
 * .disabled-container {background:Transparent;filter:Mask(Color=#010101);}"+
 ...
 * ".menu-body .separator-line {overflow:hidden;border-top:1px solid
 * ThreeDShadow;border-bottom:1px solid ThreeDHighlight;height:2px;}"+
 * ".menu-body #scroll-up-item td,.menu-body #scroll-down-item td
 * {font-family:Webdings;text-align:center;padding:10px;}"+ ".menu-body
 * #scroll-up-item,.menu-body #scroll-down-item {width:100%;}"+ ".menu-body
 * #scroll-up-item td,.menu-body #scroll-down-item td
 * {font-family:Webdings;text-align:center;padding:0px 5px 0px
 * 5px;font-size:10px;}"+ ".menu-bar
 * {background:#0257CC;cursor:default;padding:0px 1px;}"+ ".menu-bar
 * .menu-button {padding-right: 7px;padding-left: 7px;padding-bottom:
 * 2px;padding-top: 3px;background-color: #0257CC;border-top: 1px solid
 * #d6d3ce;border-right: 1px solid #d6d3ce;border-bottom: 1px solid
 * #d6d3ce;border-left: 1px solid
 * #d6d3ce;display:inline-block;font-size:12px;color:#FFFFFF;font-family:\"??\",
 * Arial;margin:1px;white-space:nowrap;cursor:default;}"+ ".menu-bar
 * .menu-button.active {padding:3px 7px 2px;border:1px solid;border-right:
 * #636563 1px solid;border-top: #636563 1px solid;border-left: #636563 1px
 * solid;cursor: default;border-bottom: #636563 1px solid;background-color:
 * #0257CC;}"+ ".menu-bar .menu-button.hover {background:blue;padding:3px 7px
 * 2px;border:1px solid;border-right: #08246b 1px solid;border-top: #08246b 1px
 * solid;border-left: #08246b 1px solid;border-bottom: #08246b 1px
 * solid;background-color: #0000FF;}"+ ".menu-bar
 * .menu-button.disabled,.menu-bar .menu-button.disabled-hover
 * {color:GrayText;border:0 ;padding:3px 7px 2px;}";
 */
...
Menu.prototype.toHtml = function() {
	...
	return "<html><head>"
			+ (this.cssText == null ? "<link type=\"text/css\" rel=\"StyleSheet\" href=\""
					+ this.cssFile + "\" />"
...
Menu.prototype.createPopup = function() {
	...
	if (pm == null) {
		...
		var d = pm.getDocument();
		w = d.parentWindow ? d.parentWindow : d.defaultView;
	}
...
Menu.prototype.areSizesCached = function() {
	...
	return this._drawn && "preferredWidth" in cs && "preferredHeight" in cs
			&& "insetLeft" in cs && "insetRight" in cs && "insetTop" in cs
			&& "insetBottom" in cs;
};
...
Menu.prototype.fixScrollButtons = function() {
	...
	if (scrollContainer.scrollHeight > this.getHeight()) {
		...
		scs.height = Math.max(0, this.getHeight()
				- (d.body.scrollHeight - scrollContainer.offsetHeight))
				+ "px";
		scs.overflow = "hidden";
...
Menu.prototype.makeEventListeners = function() {
	...
	this.eventListeners = {
		onscroll : new Function("eventListeners.menu.onscroll(\"" + this.id
				+ "\")"),
		onmouseover : new Function("eventListeners.menu.onmouseover(\""
				+ this.id + "\")"),
		...
		onmouseup : new Function("eventListeners.menu.onmouseup(\"" + this.id
				+ "\")"),
		onmousewheel : new Function("eventListeners.menu.onmousewheel(\""
				+ this.id + "\")"),
		onreadystatechange : new Function(
				"eventListeners.menu.onreadystatechange(\"" + this.id + "\")"),
		onkeydown : new Function("eventListeners.menu.onkeydown(\"" + this.id
				+ "\")"),
		oncontextmenu : new Function("eventListeners.menu.oncontextmenu(\""
				+ this.id + "\")"),
...
Menu.prototype.detachEvents = function() {
	...
	if (d) {
		...
		// scrollContainer.detachEvent("onscroll",
		// this.eventListeners.onscroll);
		...
		// linkEl.detachEvent("onreadystatechange",
		// this.eventListeners.onreadystatechange);
		// }
		// d.detachEvent("onkeydown", this.eventListeners.onkeydown);
		// d.detachEvent("oncontextmenu", this.eventListeners.oncontextmenu);

		EventUtil.removeHandler(scrollContainer, "scroll",
				this.eventListeners.onscroll);
		EventUtil
				.removeHandler(d, "mouseover", this.eventListeners.onmouseover);
		EventUtil.removeHandler(d, "mouseout", this.eventListeners.onmouseout);
		EventUtil.removeHandler(d, "mouseup", this.eventListeners.onmouseup);
		EventUtil.removeHandler(d, "mousewheel",
				this.eventListeners.onmousewheel);
		if (this.cssText == null) {
			var linkEl = d.getElementsByTagName("LINK")[0];
			EventUtil.removeHandler(linkEl, "readystatechange",
					this.eventListeners.onreadystatechang);

		}
		EventUtil.removeHandler(d, "keydown", this.eventListeners.onkeydown);
		EventUtil.removeHandler(d, "contextmenu",
				this.eventListeners.oncontextmenu);

	}
	// prevent IE to keep menu open when navigating away
	// window.detachEvent("onunload", this.eventListeners.onunload);
	EventUtil.removeHandler(window, "unload", this.eventListeners.onunload);
}
...
Menu.prototype.hookupMenu = function(d) {
	...
	EventUtil.addHandler(scrollContainer, "scroll",
			this.eventListeners.onscroll);
	EventUtil.addHandler(d, "mouseover", this.eventListeners.onmouseover);
	EventUtil.addHandler(d, "mouseout", this.eventListeners.onmouseout);
	EventUtil.addHandler(d, "mouseup", this.eventListeners.onmouseup);
	EventUtil.addHandler(d, "mousewheel", this.eventListeners.onmousewheel);
	...
	if (this.cssText == null) {
		...
		if (linkEl.readyState != "complete") {
			linkEl.attachEvent("onreadystatechange",
					this.eventListeners.onreadystatechange);
		}
	}
	...
	EventUtil.addHandler(d, "keydown", this.eventListeners.onkeydown);
	EventUtil.addHandler(d, "contextmenu", this.eventListeners.oncontextmenu);
	EventUtil.addHandler(window, "unload", this.eventListeners.onunload);
...
Menu.prototype._checkCloseState = function() {
	...
	if (closed && this._closed != closed) {
		...
		if (typeof this._onclose == "function") {
			...
			var d = this.getDocument();
			var pWindow = d.parentWindow ? d.parentWindow : d.defaultView;
			;
			var e = pWindow.event ? pWindow.event : event;
...
function MenuItem(sLabelText, fAction, sIconSrc, oSubMenu, funcid) {
	...
	this._useInsets = true; // should insets be taken into account when showing
	// sub menu
...
MenuItem.prototype.toHtml = function() {
	...
	return "<tr" + (cssClass != "" ? " class=\"" + cssClass + "\"" : "")
			+ (toolTip != "" ? " title=\"" + toolTip + "\"" : "")
			+ (!this.visible ? " style=\"display: none\"" : "") + ">"
			+ this.getIconCellHtml() + this.getTextCellHtml()
			+ this.getShortcutCellHtml() + this.getSubMenuArrowCellHtml()
			+ "</tr>";
};
...
MenuItem.prototype.getSubMenuArrowCellHtml = function() {
	return "<td class=\"arrow-cell\">"
			+ this.makeDisabledContainer(this.getSubMenuArrowHtml()) + "</td>";
};
			...
			if (this.target == "blank") {
				...
				if (Menu_postFuncId) {
					try {
						...
						// }
						// form.submit();
						window.parent.parent.doWork(this.funcid, this.text,
								this.action);
					} catch (e) {
						alert("menu.js:" + e);
					}
...
MenuItem.prototype.positionSubMenu = function() {
	...
	if (!sm._isCssFileLoaded()) {
		window.setTimeout("eventListeners.menuItem.onpositionsubmenutimer(\""
				+ this.id + "\")", 1);
		return;
	}
	...
	if (dir == "vertical") {
		if (rtl
				&& (rect.left + rect.width + menuRect.insetRight
						- menuRect.width >= 0))
			left = rect.left + rect.width + menuRect.insetRight
					- menuRect.width;
		else if (!rtl
				&& (rect.left + menuRect.width - menuRect.insetLeft <= screenWidth))
		...
		if (rect.top + rect.height + menuRect.height - menuRect.insetTop <= screenHeight)
			...
			var sizeBelow = screenHeight - rect.top - rect.height
					+ menuRect.insetTop;
		...
		else if (rect.top + rect.height - menuRect.height
				+ menuRect.insetBottom >= 0)
			top = rect.top + rect.height - menuRect.height
					+ menuRect.insetBottom;
		...
		if (rtl && (rect.left + menuRect.insetRight - menuRect.width >= 0))
			left = rect.left + menuRect.insetRight - menuRect.width;
				...
				&& (rect.left + rect.width + menuRect.width
...
MenuSeparator.prototype.toHtml = function() {
	return "<tr class=\"" + this.getCssClass() + "\""
			+ (!this.visible ? " style=\"display: none\"" : "")
			+ "><td colspan=\"4\">" + "<div class=\"separator-line\"></div>"
			+ "</td></tr>";
};
...
MenuBar.prototype.getPreferredWidth = function() {
	...
	return sw + parseInt(el.style.borderLeftWidth)
			+ parseInt(el.style.borderRightWidth);
};
...
MenuBar.prototype.getPreferredHeight = function() {
	...
	el.runtimStyle.whiteSpace = "";
	return sw + parseInt(el.style.borderTopWidth)
			+ parseInt(el.style.borderBottomWidth);
};
...
MenuBar.prototype.getInsetLeft = function() {
	return parseInt(this._htmlElement.style.borderLeftWidth);
};
MenuBar.prototype.getInsetRight = function() {
	return parseInt(this._htmlElement.style.borderRightWidth);
};
MenuBar.prototype.getInsetTop = function() {
	return parseInt(this._htmlElement.style.borderTopWidth);
};
MenuBar.prototype.getInsetBottom = function() {
	return parseInt(this._htmlElement.style.borderBottomWidth);
};
...
MenuBar.prototype._getComputedRtl = function() {
	if (this._htmlElement)
		return this._htmlElement.style.direction == "rtl";
	return false; // fall back
};
...
MenuBar.prototype.makeEventListeners = function() {
	...
	this.eventListeners = {
		onmouseover : new Function("eventListeners.menuBar.onmouseover(\""
				+ this.id + "\")"),
		onmouseout : new Function("eventListeners.menuBar.onmouseout(\""
				+ this.id + "\")"),
		onmousedown : new Function("eventListeners.menuBar.onmousedown(\""
				+ this.id + "\")"),
		onkeydown : new Function("eventListeners.menuBar.onkeydown(\""
				+ this.id + "\")"),
...
MenuBar.prototype.detachEvents = function() {
	...
	// this._htmlElement.detachEvent("onmouseout",
	// this.eventListeners.onmouseout);
	...
	EventUtil.removeHandler(this._htmlElement, "mouseover",
			this.eventListeners.onmouseover);
	EventUtil.removeHandler(this._htmlElement, "mouseout",
			this.eventListeners.onmouseout);
	EventUtil.removeHandler(this._htmlElement, "mousedown",
			this.eventListeners.onmousedown);
	EventUtil.removeHandler(this._document, "keydown",
			this.eventListeners.onkeydown);
	EventUtil.removeHandler(window, "unload", this.eventListeners.onunload);
...
MenuBar.prototype.hookupMenu = function(element) {
	...
	EventUtil.addHandler(element, "mouseover", this.eventListeners.onmouseover);
	EventUtil.addHandler(element, "mouseout", this.eventListeners.onmouseout);
	EventUtil.addHandler(element, "mousedown", this.eventListeners.onmousedown);
	EventUtil.addHandler(this._document, "keydown",
			this.eventListeners.onkeydown);
	EventUtil.addHandler(window, "unload", this.eventListeners.onunload);
};
...
MenuBar.prototype.goToNextMenuItem = function() {
	...
	var mi = this.items[this.getSelectedIndex()];
	if (expand && mi != null) {
		window.setTimeout("eventListeners.menuBar.ongotonextmenuitem(\""
				+ this.id + "\")", 1);
	}
};
...
MenuBar.prototype.goToPreviousMenuItem = function() {
	...
	var mi = this.items[this.getSelectedIndex()];
	if (expand && mi != null) {
		window.setTimeout("eventListeners.menuBar.ongotopreviousmenuitem(\""
				+ this.id + "\")", 1);
	}
};
...
function MenuButton(sLabelText, oSubMenu, faction) {
	...
	// private
	this._hover = false;
	this._useInsets = false; // should insets be taken into account when
	// showing sub menu
}
...
MenuButton.prototype.toHtml = function() {
	...
	if (this.subMenu && !this.subMenu._onclose)
		this.subMenu._onclose = new Function(
				"eventListeners.menuButton.onclose(\"" + this.id + "\")");
	...
	return "<span unselectable=\"on\" "
			...
			+ this.getTextHtml() + "</span>"
			+ "<span unselectable=\"on\" class=\"right\"></span>" + "</span>";
};
...
var eventListeners = {
	menu : {
		onkeydown : function(id) {
			/*
			 * var oThis = menuCache[id]; //delete key event by
			 * liu----------------------------------- var w =
			 * oThis.getDocument().parentWindow; oThis.handleKeyEvent( w.event );
			 */
		},
		...
		onmouseout : function(id) {
			...
			if (fromEl != null && toEl != fromEl) {
				...
				if (id == "scroll-up-item" || id == "scroll-down-item") {
					if (fromEl.className == "disabled-hover"
		...
		onmousewheel : function(id) {
			...
			var w = d.parentWindow ? d.parentWindow : d.defaultView;
			var scrollContainer = d.getElementById("scroll-container");
			scrollContainer.scrollTop -= 3 * w.event.wheelDelta / 120
					* ScrollButton.scrollAmount;
		},
	...
	menuItem : {
		onshowtimer : function(id) {
			...
			pm.closeAllSubs(sm);
			window.setTimeout("eventListeners.menuItem.onshowtimer2(\"" + id
					+ "\")", 1);
		},
	...
	menuBar : {
		onmouseover : function(id) {
			...
			var document = oThis.getDocument();
			var pWindow = document.parentWindow ? document.parentWindow
					: document.defaultView;
			var e = pWindow.event ? pWindow.event : event;
			var frome = e.fromElement;
			var fromEl = getMenuItemElement(frome);
		...
		onmousedown : function(id) {
			...
			if (el != null) {
				...
				if (mb.subMenu) {
					...
					if (new Date() - mb.subMenu._closedAt > 100) { // longer
						// than the
						// time to
						// do the hide
						mb.dispatchAction();
