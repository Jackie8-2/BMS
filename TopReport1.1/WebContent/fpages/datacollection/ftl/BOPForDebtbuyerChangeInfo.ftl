...
<@CommonQueryMacro.page title="签约信息">
	<@CommonQueryMacro.CommonQueryTab id="BOPForDebtBuyerTabs" navigate="false" currentTab="BOPForDebtbuyerChangeInfo">
	...
	<script language="JavaScript">
		...
		function initCallGetter_post()
		{
			...
			changFileType = "AB";
			BOPForDebtChangeInfo_dataset.setParameter("changFileType","AB");
		}
	    ...
	    function datatable1_filler2_onRefresh(cell,value,record) {
			if (record) {//当存在记录时
				var id = record.getValue("id");
				//var exdebtcode = record.getValue("filler2");
		...
		//新增
		function btNewClick(){
			showWin("买方信贷变动信息","${contextPath}/fpages/datacollection/ftl/BOPForDebtBuyerChangeInfoCol.ftl?op=new&changFileType="+changFileType,"report","flushPage()");
		}
		//详细
		function doDetail(id) {
			locate(id);
			showWin("买方信贷变动信息","${contextPath}/fpages/datacollection/ftl/BOPForDebtBuyerChangeInfoCol.ftl?id="+id+"&op=detaile&changFileType="+changFileType,"report","flushPage()");
		}
		...
		function doModify(id) {
		    locate(id);
			showWin("买方信贷变动信息","${contextPath}/fpages/datacollection/ftl/BOPForDebtBuyerChangeInfoCol.ftl?id="+id+"&op=mod&changFileType="+changFileType,"report","flushPage()");
		}

		//删除
		function doDelete(id) {
		    locate(id);
			showWin("买方信贷变动信息","${contextPath}/fpages/datacollection/ftl/BOPForDebtBuyerChangeInfoCol.ftl?id="+id+"&op=del&changFileType="+changFileType,"report","flushPage()");
		}
