...
<@CommonQueryMacro.page title="BOPForDebtBilLoan.title">
	<table width="100%">
		<tr>
			<td width="100" valign="top">
				<tr>
					<td>
						<@CommonQueryMacro.CommonQuery id="BOPForDebtNobleMetalLendingCol" init="true" submitMode="all" navigate="false">
	...
	<script language="javascript">
		...
		{
			var approveStatus = BOPForDebtNobleMetalLendingCol_dataset.getValue("approveStatus");
			var repStatus = BOPForDebtNobleMetalLendingCol_dataset.getValue("repStatus");
			if (repStatus != "02") {
			...
			{
				window.document.getElementById('btColDel').style.display="none";
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("actiondesc",true);
			}
			else if("del" == op)
			{
			    BOPForDebtNobleMetalLendingCol_dataset.getField("actiondesc").required=true;
	
				window.document.getElementById('btColSave').style.display="none";
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("creditortype",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("debtorcode",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("creditorcode",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("debtype",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("creditorname",true);
	
				//BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("contractdate",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("creditornamen",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("contractcurr",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("crehqcode",true);
	
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("contractamount",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("opercode",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("valuedate",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("maturity",true);
	
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("floatrate",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("anninrate",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("inprterm",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("spapfeboindex",true);
	
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("remark",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("filler2",true);
			...
			else if("detaile" == op)
			{
				window.document.getElementById('btColDel').style.display="none";
				window.document.getElementById('btColSave').style.display="none";
				BOPForDebtNobleMetalLendingCol_dataset.readOnly=true;
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("creditortype",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("debtorcode",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("creditorcode",true);
				BOPForDebtNobleMetalLendingCol_dataset.setFieldReadOnly("debtype",true);
	    ...
	    function doDelete()
		{
			...
			//定位一条记录
		function locate(id) {
			var record = BOPForDebtNobleMetalLendingCol_dataset.find(["id"],[id]);
			if (record) {
				BOPForDebtNobleMetalLendingCol_dataset.setRecord(record);
			}
		}
		...
		{
			var anninrate = BOPForDebtNobleMetalLendingCol_dataset.getValue("anninrate");
			if (!isNaN(anninrate) && parseFloat(anninrate) >= 1) {
				alert("[年化利率值]必须为小数");
				return false;
			}
	
			if(isEmtry(BOPForDebtNobleMetalLendingCol_dataset.getValue('creditortype') ))
			{
				alert("字段[ 债务类型 ] 不能为空!");
				return false;
			}
			if(isEmtry(BOPForDebtNobleMetalLendingCol_dataset.getValue('creditorname') ) && isEmtry(BOPForDebtNobleMetalLendingCol_dataset.getValue('creditornamen') ))
			{
				alert("债权人中文名称和债权人英文名称至少填写一个!");
				return false;
			}
	
			if (!BOPForDebtNobleMetalLendingCol_dataset.modified) 
			{
				alert("请先修改后再保存！");
				return false;
			}
		...
		function btColDel_onClickCheck(button)
		{
			if (op == "del") 
			{
				var actiondesc = BOPForDebtNobleMetalLendingCol_dataset.getValue("actiondesc");
				if (actiondesc.length == 0) 
				{
					alert("请填写删除原因！");
					return false;
				}
				BOPForDebtNobleMetalLendingCol_dataset.setParameter("op", op);
			}
		}
	
		function doRepDet()
		{
			var id = BOPForDebtNobleMetalLendingCol_dataset.getValue("id");
			var appType = BOPForDebtNobleMetalLendingCol_dataset.getValue("appType");
			var currentfile = BOPForDebtNobleMetalLendingCol_dataset.getValue("currentfile");
			var busiCode = BOPForDebtNobleMetalLendingCol_dataset.getValue("exdebtcode");
