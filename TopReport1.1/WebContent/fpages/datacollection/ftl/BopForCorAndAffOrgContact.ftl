...
<@CommonQueryMacro.page title="签约信息">
	...
	<script language="JavaScript">
		...
		//新增
		function btNewClick(){
			showWin("境外联行及附属机构往来签约新增","${contextPath}/fpages/datacollection/ftl/BopForCorAndAffOrgContactInfo.ftl?op=new","report","flushPage()");
		}
		//刷新数据
		function flushPage(){
		...
		function doCollection(id){
			showWin("境外联行及附属机构往来签约信息修改","${contextPath}/fpages/datacollection/ftl/BopForCorAndAffOrgContactInfo.ftl?id=" + id + "&op=modify","report","flushPage()");
		}
		//删除
		function doDelete(id){
			showWin("境外联行及附属机构往来签约信息删除","${contextPath}/fpages/datacollection/ftl/BopForCorAndAffOrgContactInfo.ftl?id=" + id + "&op=delete","report","flushPage()");
		}
		//详细信息
		function doDetail(id){
			showWin("境外联行及附属机构往来签约信息明细","${contextPath}/fpages/datacollection/ftl/BopForCorAndAffOrgContactInfo.ftl?id=" + id + "&op=detail");
		}
	</script>
