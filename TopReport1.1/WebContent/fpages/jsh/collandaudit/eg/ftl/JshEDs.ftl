...
<@CommonQueryMacro.page title="外汇账户内购汇基础信息">
	<@CommonQueryMacro.CommonQueryTab id="JshEgDsTabs" navigate="false" currentTab="JshEDs">
		<@CommonQueryMacro.CommonQuery id="JshEDs" init="false" submitMode="all" navigate="false" >
			<table align="left">
		    	...
		    	<tr>
		    		<td colspan="2">
						<@CommonQueryMacro.DataTable id ="datatable1" paginationbar="btAdd" fieldStr="opr[80],filler2[80],buscode[80],brNo[80],brNoName[80],workDate[80],recStatus[80],approveStatus[80],repStatus[80],actiontype[80],actiondesc[80],rptno[80],custype,custcod,idcode,custnm,lcyacc,fcyacc,oppuser,oppbank,lcyamt,lcyccy,exrate" hasFrame="true" width="900" height="260" readonly="true"/>
		      		</td>
		    	</tr>
			</table>
	...
	<script language="JavaScript">
		...
		function locate(id) {
			var record = MtsJshEDs_dataset.find(["id"],[id]);
			if (record) {
				MtsJshEDs_dataset.setRecord(record);
			}
		}
		function btNewClick(){
		
			showWin("外汇账户内购汇基础信息","${contextPath}/fpages/jsh/collandaudit/eg/ftl/JshEDsAdd.ftl?op=new","window","flushPage()",window);
		}
		//修改
		function doCollection(id){
			showWin("外汇账户内购汇基础信息修改","${contextPath}/fpages/jsh/collandaudit/eg/ftl/JshEDsAdd.ftl?id=" + id + "&op=mod","window","flushPage()",window);
		}
		//删除
		function doDelete(id){
			showWin("外汇账户内购汇基础信息删除","${contextPath}/fpages/jsh/collandaudit/eg/ftl/JshEDsAdd.ftl?id=" + id + "&op=del","window","flushPage()",window);
		}
		//详细信息
