...
<@CommonQueryMacro.page title="申报信息">
	...
	<script language="JavaScript">
		...
		//新增
		function btNewClick(){
			showWin("管理信息新增","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopSDsCollectionInfo.ftl?op=new","window","flushPage()",window);
		}
		...
		function doCollection(id){
			showWin("管理信息修改","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopSDsCollectionInfo.ftl?id=" + id + "&op=modify","window","flushPage()",window);
		}

		//删除
		function doDelete(id){
			showWin("管理信息删除","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopSDsCollectionInfo.ftl?id=" + id + "&op=delete","window","flushPage()",window);
		}

		//详细信息
		function doDetail(id){
			showWin("管理信息明细","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopSDsCollectionInfo.ftl?id=" + id + "&op=detail","window","",window);
		}
	</script>
