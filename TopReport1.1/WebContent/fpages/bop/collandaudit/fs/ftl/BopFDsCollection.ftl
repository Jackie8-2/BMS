...
<@CommonQueryMacro.page title="基础信息">
	<@CommonQueryMacro.CommonQueryTab id="BopFSDsCollectionTabs" navigate="false" currentTab="BopFDsCollection">
		<@CommonQueryMacro.CommonQuery id="BopFDsCollection" init="false" submitMode="current" navigate="false" >
			<table align="left">
				<tr>
					<td colspan="2">
	...
	<script language="JavaScript">
		...
		function initCallGetter_post(){
			...
			BopFDsCollection_interface_dataset.setValue("qworkDateStart", currentDate);
			BopFDsCollection_interface_dataset.setValue("qworkDateEnd", currentDate);
		}
		...
		//新增
		function btNewClick(){
			showWin("基础信息新增","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopFDsCollectionInfo.ftl?op=new","window","flushPage()",window);
		}

		//刷新数据
		function flushPage(){
			BopFDsCollection_dataset.flushData(1);
		}

		//修改
		function doCollection(id){
			showWin("基础信息修改","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopFDsCollectionInfo.ftl?id=" + id + "&op=modify","window","flushPage()",window);
		}

		//删除
		function doDelete(id){
			showWin("基础信息删除","${contextPath}/fpages/bop/collandaudit/fs/ftl/BopFDsCollectionInfo.ftl?id=" + id + "&op=delete","window","flushPage()",window);
		}
